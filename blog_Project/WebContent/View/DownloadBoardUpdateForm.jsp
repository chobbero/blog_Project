...
<%
	...
	DownloadBoardBean dbb = dbdao.getDownloadBoard(idx);
	
	if (!id.equals( dbb.getId())) {
		%><script>
			alert("글작성자만 수정가능!");
			history.back();
		</script><%	
	...
	<section class="view">
		...
		<section class="boardView">
			<div class="boardWriteForm">
				<form action="../Process/DownloadBoardUpdatePro.jsp" method="post" enctype="Multipart/form-data" id="contentsWriteForm">
					<div>
						<table>
							<tr>
								<td class="wTitle">아이디</td>
								<td id="WriteIdPass"><input type="text" name="id" value="<%=dbb.getId()%>" id="blockIdPass" readonly></td>
							...
							<tr>
								...
								<td id="WriteTitle" colspan="3"><input type="text" name="title" maxlength="100" value="<%=dbb.getTitle()%>" id="blockTitle" required></td>
							</tr>
							
							<tr>
								<td class="wTitle">내용</td>
								<td id="WriteContents" colspan="3"><textarea name="contents" id="blockContents" maxlength="1000"  required><%=dbb.getContents()%></textarea></td>
							</tr>
